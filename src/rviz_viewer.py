...
def goal_marker_cb(msg, args):
    server = args[0]
    p = msg.ee_poses[0]
    pos_goal = numpy.array(args[1]) + numpy.array(
        [p.position.x, p.position.y, p.position.z]
    )
    rot_goal = T.quaternion_multiply(
...
def print_cb(msg):
    p = msg.pose.position
    print(
        msg.marker_name + " is now at [" + str(p.x) + ", " + str(p.y) + ", "
        + str(p.z) + "]"
    )
...
def make_marker(
    ...
    if shape == 'widget':
        ...
        colors = [
            [1.0, 0.0, 0.0, 0.6], [0.0, 1.0, 0.0, 0.6], [0.0, 0.0, 1.0, 0.6]
        ]
    ...
    if 'cuboids' in obstacles:
        ...
        if cuboids is not None:
            for c in cuboids:
                ...
                c_quat = T.quaternion_from_euler(
                    c['rotation'][0], c['rotation'][1], c['rotation'][2]
                )
                int_marker = make_marker(
                    c['name'], fixed_frame, "cuboid", c['scale'],
                ...
                if is_dynamic == 2:
                    ...
                    waypoints = utils.get_abs_waypoints(
                        relative_waypoints, int_marker.pose
                    )
        ...
        if spheres is not None:
            for s in spheres:
                if s['animation'] == 'static':
                    ...
                    is_dynamic = 2
                int_marker = make_marker(
                    s['name'], fixed_frame, "sphere", [s['scale']] * 3,
        ...
        if point_cloud is not None:
            for pc in point_cloud:
                ...
                if pc['animation'] == 'static':
                    ...
                    is_dynamic = 2
                pc_quat = T.quaternion_from_euler(
                    pc['rotation'][0], pc['rotation'][1], pc['rotation'][2]
                )
                int_marker = make_marker(
                    pc['name'],
                    fixed_frame,
                    "point_cloud", [0.01, 0.01, 0.01],
                    pc['translation'],
                    pc_quat,
                    is_dynamic,
                    points=pc_points
...
def main():
    ...
    joint_state_define_file = open(
        path_to_src + '/relaxed_ik_core/config/joint_state_define_functions/'
        + joint_state_define_file_name, 'r'
    )
    joint_state_define = joint_state_define_file.read()
    exec(joint_state_define)

    urdf_file = open(
        path_to_src + '/relaxed_ik_core/config/urdfs/' + urdf_file_name, 'r'
    )
    urdf_string = urdf_file.read()
    rospy.set_param('robot_description', urdf_string)
    js_pub = rospy.Publisher('joint_states', JointState, queue_size=5)
    rospy.Subscriber(
        '/ur5/relaxed_ik/joint_angle_solutions', JointAngles, ja_solution_cb
    )
    ...
    pose_goal_marker = make_marker(
        'pose_goal', fixed_frame, 'widget', [0.1, 0.1, 0.1], init_pos, init_rot,
        False
    )
    server.insert(pose_goal_marker)

    rospy.Subscriber(
        '/ur5/relaxed_ik/ee_pose_goals', EEPoseGoals, goal_marker_cb,
        (server, init_pos, init_rot)
    )
    rospy.Subscriber('/ur5/relaxed_ik/current_time', Float64, time_update_cb)
        ...
        if initialized:
            ...
            for (name, waypoints) in dyn_obs_handles:
                if time_cur < len(waypoints) * delta_time:
                    (time, pose) = utils.linear_interpolate_waypoints(
                        waypoints, int(time_cur / delta_time)
                    )
